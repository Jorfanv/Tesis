...
def cargar_datos_tiendas():
    df = pd.read_csv(r"..\scripts\data\proccesed\tiendas.csv")
    return df
...
def intercambios_sugeridos(df):
    ...
    # índice auxiliar para facilitar el cruce
    df_filtered['id'] = df_filtered['NUMERO DE IDENTIFICACION']

    # subconjunto con las columnas necesarias para comparar
    candidatos = df_filtered[[
        'id', 'NOMBRE COMPLETO', 'LUGAR DE TRABAJO', 'tienda_mas_cercana_diferente',
        'DESCRIPCION PUESTO', 'mejora_%'
    ]].copy()

    # se renombra columnas para facilitar el merge posterior
    candidatos.columns = [
        'id_1', 'nombre_1', 'tienda_actual_1', 'tienda_sugerida_1', 'puesto_1', 'mejora_1'
    ]

    # merge del DataFrame consigo mismo para encontrar pares cruzados
    intercambios = pd.merge(
        candidatos, candidatos,
        left_on=['tienda_sugerida_1', 'tienda_actual_1', 'puesto_1'],
        right_on=['tienda_actual_1', 'tienda_sugerida_1', 'puesto_1'],
        suffixes=('_a', '_b')
    )

    # Filtrar para que no se compare un empleado consigo mismo y ambos tengan mejora positiva
    ...
    # # Eliminar duplicados (misma pareja invertida)
    # intercambios['par'] = intercambios.apply(
    #     lambda row: tuple(sorted([row['id_1_a'], row['id_1_b']])), axis=1
    # )
    # #intercambios = intercambios.drop_duplicates(subset='par').drop(columns='par')
...
def antiguedad(fecha_ingreso):
    if fecha_ingreso is None:
        ...
        return round((fecha_actual - fecha_ingreso).days / 365, 2)

# Función de lectura de archivo 
# @st.cache_data
def cargar_datos():
    dir_distancias = r"..\scripts\data\distancias"
    n, lista = contar_y_ultimo_elemento(dir_distancias)
    dir_df = sorted(list)[-1]
    df = pd.read_csv(dir_distancias + "\\" + dir_df)
    return df